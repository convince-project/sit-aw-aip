...
class UC3(SIMD):
    ...
    def read_extract_from_bag(self, data_path, reader, topic_types):
        ...
        while reader.has_next():
            ...
            if topic == "/DialogComponent/ServiceEvents":
                msg_type = get_message(topic_types[topic])
                msg = deserialize_message(msg_data, msg_type)
                if hasattr(msg, 'data') and len(str(msg.data))>0:
                    service_events.append(str(msg.data))
            ...
            if topic == "/NavigationComponent/GoToPoi/_action/feedback":
                msg_type = get_message(topic_types[topic])
                msg = deserialize_message(msg_data, msg_type)
                if hasattr(msg, 'feedback') and hasattr(msg.feedback, 'navigation_status'):
                    feedback_statuses.append(msg.feedback.navigation_status)
        ...
        with open(f"{data_path}/text_files/navigation_status_evolution.txt", "w") as file:
            file.write(f"Those represent the navigation status evolution in time:\n")
            for s in range(len(st_evolution)):
                file.write(f"status_{s} : {st_evolution[s]}\n")

        if len(feedback_statuses) > 0:
            with open(f"{data_path}/text_files/navigation_feedback.txt", "w") as file:
                file.write(f"Navigation feedback status over time (from GoToPoi feedback):\n")
                for s in range(len(feedback_statuses)):
                    file.write(f"feedback_{s} : {feedback_statuses[s]}\n")
        ...
        if len(service_events) != 0:
            with open(f"{data_path}/text_files/service_events.txt", "w") as file:
                file.write(f"Those are the dialog service events (robot responses) in order:\n")
                for s in range(len(service_events)):
                    file.write(f"event_{s} : {service_events[s]}\n")
